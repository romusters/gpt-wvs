...
def save_results(df):
    ...
    blob_client = blob_service_client.get_blob_client(container="data", blob=f"results\verschil\{datetime.now()}_{prefix}_result.csv")
    csv_buffer = StringIO()
...
def show_question():
    ...
    if current_index < len(df):
        ...
        col5.write(f"**AI oordeel:** {row['verschil_uitleg']}")
